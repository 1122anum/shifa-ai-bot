...
class TestHandleVoiceMessage:
    ...
    def test_voice_full_flow(self, mock_dl, mock_vt, mock_send, mock_clean):
        ...
        mock_vt.assert_called_once_with(user_id=FROM, audio_path="audio_temp_test/test.ogg")
    ...
    @patch("app.handlers.voice_handler.call_voice_triage")
    @patch("app.handlers.voice_handler.download_audio")
    def test_transcription_failure(self, mock_dl, mock_vt, mock_send, mock_clean):
    ...
    @patch("app.handlers.voice_handler.call_voice_triage")
    @patch("app.handlers.voice_handler.download_audio")
    def test_backend_failure(self, mock_dl, mock_vt, mock_send, mock_clean):
        from app.services.backend_client import BackendError
        mock_dl.return_value = "audio_temp_test/test.ogg"
    ...
    @patch("app.handlers.voice_handler.call_voice_triage")
    @patch("app.handlers.voice_handler.download_audio")
    def test_emergency_formatting(self, mock_dl, mock_vt, mock_send, mock_clean):
        mock_dl.return_value = "audio_temp_test/test.ogg"
        mock_vt.return_value = {
